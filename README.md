...
- init-with-renaming.sh: 초기 설정시 폴더명과 특정 앱 명칭 및 코드상에서의 내용을 일괄로 변경하기 위한 스크립트 (las_api -> gymt)
...
## **3. init-with-renaming.sh을 실행하여 메인 앱 폴더 및 소스코드내 명칭 일괄 변경 (las_api -> 프로젝트명칭)** ##
해당 프로젝트는 앱 명칭 및 그에 다른 소스코드내 명칭들이 모두 startkit or las_api로 되어있기 때문에
이를 일괄로 변경시키는 스크립트를 실행하여 한번에 처리 (ex. las_api -> gymt)
